...
@rem     echo "function prompt { $p = Split-Path -leaf -path (Get-Location); Write-Output $p> }" > home\user-profile\Documents\WindowsPowerShell\Microsoft.PowerShell_profile.ps1
)
...
@rem     call bin\code.cmd --extensions-dir .\data\extension --user-data-dir .\data\user-data --install-extension dart-code.dart-code 2>nul
    call bin\code.cmd --extensions-dir .\data\extension --user-data-dir .\data\user-data --install-extension dart-code.flutter 2>nul
...
copy "Flutter Code.lnk" %UserProfile%\desktop /y 1>nul
